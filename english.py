...
def visualizations_page():
    ...
    language = col4.selectbox("Preferred Language", ["English", "Telugu"],index=0 if user_data[26]=='English' else 1 if user_data[26]=='Telugu' else 2)
...
def english_page():
    ...
    with st.sidebar:
        ...
        selected_tab = option_menu(
            menu_title=None,
            options=["User Profile", "Update Page", "Loans", 'Segmentation','Fraud Detection','Logout'],
    ...
    if selected_tab == "User Profile":
        ...
        st.markdown('<h1 style="color: #9c0399; text-align: center;">Customer Segmentation</h1>', unsafe_allow_html=True)
        st.markdown(" ")
        ...
        st.markdown(
            ...
            <div class="description-box">
                <span class="segment">You Category belongs to {seg}</span><br><br>
        ...
        segment_labels = ['Starter', 'Builder', 'Elite', 'Seeker']
        ...
        fig = px.bar(
            ...
            text="Probability",
            labels={"Segment": "Profession Segments", "Probability": "Probability"},
        ...
        st.markdown('<h1 style="color: #9c0399; text-align: center;">Fraud Detection</h1>', unsafe_allow_html=True)
        st.write("")
        st.write("")
        ...
        merchant = 1
        col1,col2,col3=st.columns(3)
        mode=col1.selectbox('Choose the mode of transaction',['Online','Offline'])
        if mode=='Online':
            category = 1
        else:
            category=0
        portal=col2.selectbox('Choose the portal',['Bank Transfer','Credit Card','Debit Card','E-Wallet','UPI','Cash'])
        amount = col3.number_input("Transaction Amount", min_value=0.0, step=0.01)

        # Submit Button
        col1,col2,col3=st.columns([2,2,1])
        if col2.button("Detect Fraud",type='primary'):
            if portal=='UPI' and amount>5000 and mode=='Online':
                st.error('High Chances of Fraud')
            elif portal=='Credit Card' and amount>10000 and mode=='Online':
                st.error('High Chances of Fraud')
            elif portal=='Debit Card' and amount>5000 and mode=='Online':
                st.error('High Chances of Fraud')
            elif mode=='Offline' and (portal=='Credit Card' or portal=='Debit Card' or portal=='E-Wallet' and portal=='Bank Transfer' and portal=='UPI'):
                st.error('Unable to detect fraud in offline mode')
            elif mode=='Offline' and portal=='Cash' and amount>100000:
                st.error('High Chances of Fraud')
            elif mode=='Offline' and amount<100000:
                st.success('No Fraud Detected')
            elif mode=='Online' and portal=='Cash':
                st.error('Unable to detect fraud in Online mode')
            else:
                ...
                # Display Results
                if result == 1:
                    prediction = "🚨 High Chances of Fraud"
                    st.error(prediction)
                else:
                    prediction = "✅ Non-Fraudulent Payment"
